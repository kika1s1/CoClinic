  ...
  return (
    <div className='w-full  h-full grid grid-cols-[48px,1fr] bg-white'>
            <div className='bg-white w-12 h-full rounded-tr-lg rounded-br-lg py-5 text-slate-600 flex flex-col justify-between'>
                <div>
                    <NavLink to="/livechat"   className={({isActive})=>`w-12 h-12 flex justify-center items-center  cursor-pointer hover:bg-slate-200 rounded ${isActive && "bg-slate-100" }`} title='chat'>
                        <HiOutlineChat
                            size={20}
                        />
                    </NavLink>
                ...
                <div className=' h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar'>
...

                    {
                        allUser.map((conv,/*index*/)=>{
                            return(
                                <NavLink to={`/livechat/${conv?.userDetails?._id}`} key={conv?._id} className='flex items-center gap-2 py-3 px-2 border border-transparent hover:border-primary rounded hover:bg-slate-100 cursor-pointer'>
                                    <div>
                                        <Avatar
